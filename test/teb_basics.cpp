...
TEST(TEBBasic, autoResizeLargeValueAtEnd)
{
  ...
  teb_local_planner_dynamic_planning::TimedElasticBand teb;
  
  teb.addPose(teb_local_planner_dynamic_planning::PoseSE2(0., 0., 0.));
  for (int i = 1; i < 10; ++i) {
    teb.addPoseAndTimeDiff(teb_local_planner_dynamic_planning::PoseSE2(i * 1., 0., 0.), dt);
  }
  // add a pose with a large timediff as the last one
  teb.addPoseAndTimeDiff(teb_local_planner_dynamic_planning::PoseSE2(10., 0., 0.), dt + 2*dt_hysteresis);
